...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/371117/cf-fast-submit.user.js
// @updateURL https://update.greasyfork.org/scripts/371117/cf-fast-submit.meta.js
// ==/UserScript==
...
(function () {
  ...
  const extensionMap = {
    ...
    55: "program.js",
    59: "program.cpp",
    60: "[^{}]*publics+(final)?s*classs+(w+).*|$2.java",
    61: "program.cpp",
  };
  ...
  async function initAppendForm(first = true, doNotRegenerateOnSubmit = false) {
    ...
    $programType = $form.find("#programTypeId, select[name=programTypeId]");
  ...
  function setAceMode() {
    if (!$programType || !$programType.length) return;
    var val = $programType.val();
    if (!val) return;
    var filePath = extensionMap[val];
    if (!filePath) return;
