...
(function () {
  ...
  // (기존) const contentCss = "...";
  const loadingCss = `
    .carrot-loading-col { min-width: 4.5em; }
    .carrot-spinner {
    display:inline-block; width:12px; height:12px;
    border:2px solid #ccc; border-top-color:#888;
    border-radius:50%; animation: carrot-spin 1s linear infinite;
    vertical-align: middle;
    }
    @keyframes carrot-spin { to { transform: rotate(360deg); } }

    .carrot-loading-hint.small { color:#999; }
    .carrot-left-sep { border-left: 1px solid #dcdcdc; }
    th.carrot-left-sep { border-left: 1px solid #dcdcdc; }
  `;
  ...
  function updateStandings(resp) {
    ...
    for (const [idx, tableRow] of rows.entries()) {
      // 기존 마지막 셀에서 'right'를 제거해야 하는데,
      // 로딩 열(.carrot-loading-col)이 이미 붙어 있으면
      // 마지막 셀이 로딩 셀이라 보더가 꼬일 수 있음.
      // => 로딩 열을 건너뛴 '원래 마지막 셀'에서 제거한다.
      const lastNonLoading = (() => {
        let c = tableRow.lastElementChild;
        while (c && c.classList.contains("carrot-loading-col")) {
          c = c.previousElementSibling;
        }
        return c;
      })();
      if (lastNonLoading) lastNonLoading.classList.remove("right");
  ...
  // 엄격 대기 유틸: cond가 true가 될 때까지 대기(타임아웃/폴링/Observer 모두 사용)
  function waitFor(cond, { timeoutMs = 10000, pollMs = 100 } = {}) {
  ...
  async function insertLoadingColumnsAsync() {
    await waitFor(() => !!document.querySelector("table.standings"));
    ...
    await waitFor(() => {
      ...
      const contestantCell = firstDataRow.querySelector("td.contestant-cell");
      return !!contestantCell && contestantCell.textContent.trim().length > 0;
    });

    if (document.querySelector(".carrot-loading-col")) {
      return () => {};
    }
    ...
    // insertLoadingColumnsAsync() 내부, "헤더 행" 블록 전체 교체
    {
      const header = rows[0];

      // 원래 마지막 셀의 right 제거
      const lastNonLoading = (() => {
        let c = header.lastElementChild;
        while (c && c.classList.contains("carrot-loading-col"))
          c = c.previousElementSibling;
        return c;
      })();
      ...
      // 공용 헤더 생성기 (Unicode 사용)
      const makeLoadingHeaderCell = (
        symbol,
        title,
        classes = [],
        width = ""
        ...
        th.classList.add("top", "carrot-loading-col", ...classes);
        if (width) th.style.width = width;
        th.innerHTML =
          `<span title="${title}">${symbol}</span><br>` +
          `<span class="small carrot-loading-hint">Loading…</span>`;
        return th;
      };

      // Π | Δ | ▲▼  (Unicode 상수 사용)
      const thPerf = makeLoadingHeaderCell(
        Unicode.GREEK_CAPITAL_PI, // Π
        "Performance",
        ["carrot-left-sep"], // 왼쪽 구분선 유지
        "4em"
      );
      const thDelta = makeLoadingHeaderCell(
        Unicode.GREEK_CAPITAL_DELTA, // Δ
        "Delta",
        [],
        "4.5em"
      );
      const thRank = makeLoadingHeaderCell(
        Unicode.BLACK_CURVED_RIGHTWARDS_AND_UPWARDS_ARROW, // ▲▼
        "Rank up",
        ["right"], // 우측 보더 유지
        "7.5em"
      );

      header.appendChild(thPerf);
      header.appendChild(thDelta);
    ...
    for (let rIdx = 1; rIdx < rows.length; rIdx++) {
      ...
      // (A) 로딩 열을 붙이기 전에 원래 마지막 셀에서 'right' 제거
      const lastNonLoading = (() => {
        let c = tr.lastElementChild;
        while (c && c.classList.contains("carrot-loading-col"))
          c = c.previousElementSibling;
        return c;
      })();
      ...
      const isDarkRow = !!(rIdx % 2); // (B) zebra 적용

      const mkCell = (
        opts = { isRight: false, isBottom: false, isFirstLoading: false }
      ) => {
        const td = document.createElement("td");
        td.classList.add("carrot-loading-col");
        if (opts.isRight) td.classList.add("right");
        if (opts.isBottom) td.classList.add("bottom");
        if (isDarkRow) td.classList.add("dark"); // (B)
        if (opts.isFirstLoading) td.classList.add("carrot-left-sep"); // (C)
        td.innerHTML = `<span class="carrot-spinner"></span>`;
        return td;
      };

      // 첫/둘째/셋째 로딩 셀
      tr.appendChild(mkCell({ isBottom, isFirstLoading: true }));
      tr.appendChild(mkCell({ isBottom }));
      tr.appendChild(mkCell({ isRight: true, isBottom }));
    }

    await new Promise(requestAnimationFrame);

    return function removeLoadingColumns() {
      document
        .querySelectorAll(".carrot-loading-col")
        .forEach((el) => el.remove());
    };
  }

  function main() {
    _GM_addStyle(contentCss + loadingCss);
    ...
    if (contestId && document.querySelector("table.standings")) {
      (async () => {
        const removeLoading = await insertLoadingColumnsAsync();

        // 🔽 DOM 붙였으니, 브라우저에 그릴 시간을 확실히 준다
        await new Promise(requestAnimationFrame); // 1) 다음 프레임 콜백(페인트 직전)
        await new Promise(requestAnimationFrame); // 2) 한 프레임 더 양보 (안전)
        await new Promise((r) => setTimeout(r, 0)); // 3) 다음 macrotask로 넘기기

        try {
          await predict(Number.parseInt(contestId));
        } catch (er) {
